...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['admin_id']) && $_POST['admin_id'] !== "") {
        ...
        // Update admin
        if (!empty($password)) {
            $conn->query("UPDATE users SET name='$full_name', email='$email', password='$password', status='$status' WHERE id=$id");
        } else {
            // Update without changing the password
            $conn->query("UPDATE users SET name='$full_name', email='$email', status='$status' WHERE id=$id");
        }
    } else {
        // Insert new admin
        $conn->query("INSERT INTO users (full_name, username, password, status) VALUES ('$full_name', '$email', '$password', '$status')");
    }
                        ...
                        <div class="modal-body">
                            <form method="POST">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="full_name" id="full_name" class="form-control bg-light p-1"
                                        required>
                                </div>
                                <div class="mb-3">
